...
htdoc_open_document_with_buffer(fz_context *ctx, fz_archive *zip, fz_buffer *buf, int format)
{
	...
	fz_try(ctx)
	{
		...
		switch (format)
		{
		case FORMAT_FB2: doc->html = fz_parse_xhtml(ctx, doc->set, doc->zip, ".", buf, fz_user_css(ctx)); break;
...
{
	return htdoc_open_document_with_buffer(ctx, NULL, fz_read_all(ctx, file, 0), FORMAT_XHTML);
}

static fz_document *
fb2doc_open_document(fz_context *ctx, const char *filename)
{
	return htdoc_open_document_with_buffer(ctx, NULL, fz_read_file(ctx, filename), FORMAT_XHTML);
}
