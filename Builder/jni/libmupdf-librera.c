...
JNICALL Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_open(
...
JNIEXPORT void JNICALL
Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_free(
    JNIEnv
*env,
jclass clazz, jlong
dochandle,
...
JNIEXPORT void JNICALL
Java_org_ebookdroid_droids_mupdf_codec_MuPdfOutline_free(
    JNIEnv
*env,
jclass clazz, jlong
dochandle) {
...
JNICALL Java_org_ebookdroid_droids_mupdf_codec_MuPdfOutline_getTitleArray(JNIEnv *env,
...
if (!idoc) {
(*env)->
ReleaseStringUTFChars(env, fname, path
);
return;
}
...
fz_try(ctx) {
    ...
    for (i = 0; i < n; i++) {
      ...
      switch (i % 4) {
      case 0:pts[i / 4].ll = pt;
        break;
      case 1:pts[i / 4].lr = pt;
        break;
      case 2:pts[i / 4].ur = pt;
        break;
      case 3:pts[i / 4].ul = pt;
        break;
      }
...
void fz_print_stext_block_as_html_my(fz_context *ctx, fz_output *out,
  ...
  for (line = block->u.t.first_line; line; line = line->next) {
    ...
    for (ch = line->first_char; ch; ch = ch->next) {
      ...
      default:n = fz_runetochar(utf, ch->c);
        for (i = 0; i < n; i++)
