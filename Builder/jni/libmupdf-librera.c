...
{
    jclass new_exception = (*env)->FindClass(env, exception);
    if (new_exception == NULL)
    {
        DEBUG("Exception class not found: '%s'", exception);
        return;
    }
    DEBUG("Exception '%s', Message: '%s'", exception, message ? message : "");
    (*env)->ThrowNew(env, new_exception, message ? message : "Unknown error");
    (*env)->DeleteLocalRef(env, new_exception);
}

void mupdf_throw_exception(JNIEnv *env, char *message)
{
    mupdf_throw_exception_ex(env, RUNTIME_EXCEPTION, message ? message : "Unknown error");
}
...
{
    return (*env)->NewStringUTF(env, FZ_VERSION "test");
}

JNIEXPORT jlong
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfDocument_open(JNIEnv *env,
                                                              jclass clazz, jint storememory,
                                                              ...
                                                              jstring accelerate, jint is_image_scale)
{
    ...
    if (!fname || !pwd || !jcss || !accelerate) {
        mupdf_throw_exception(env, "Invalid parameters");
...
Java_org_ebookdroid_droids_mupdf_codec_MuPdfDocument_free(JNIEnv
                                                              *env,
...
JNIEXPORT jstring
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfDocument_getMeta(JNIEnv *env,
                                                                 ...
                                                                 jstring joptions)
{
    ...
    if (!doc || !doc->ctx || !doc->document || !joptions) {
...
JNIEXPORT jint
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfDocument_getPageInfo(JNIEnv *env,
                                                                     jclass cls, jlong handle,
                                                                     jint pageNumber, jobject cpi)
{

    renderdocument_t *doc = (renderdocument_t *)(long)handle;
    
    if (!doc || !doc->ctx || !doc->document || !cpi) {
        return -1;
    }
...
JNIEXPORT jlong
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfLinks_getNextPageLink(JNIEnv *env,
...
JNIEXPORT jboolean
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfLinks_fillPageLinkSourceRect(
...
JNIEXPORT jint
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfLinks_getPageLinkTargetPage(
        JNIEnv *env, jclass clazz, jlong handle, jlong linkhandle)
{
    ...
    fz_try(doc->ctx)
    {
        fz_location loc = fz_resolve_link(doc->ctx, doc->document, link->uri, NULL, NULL);
        pageNum = fz_page_number_from_location(doc->ctx, doc->document, loc);
    }
    fz_catch(doc->ctx)
    {
        pageNum = -1;
...
JNIEXPORT jint
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfLinks_getLinkPage(
        JNIEnv *env, jclass clazz, jlong handle, jstring id)
{

    renderdocument_t *doc = (renderdocument_t *)(long)handle;
    if(!doc || !doc->ctx || !doc->document || !id){
      return -1;
    }
    ...
    fz_try(doc->ctx)
    {
        fz_location loc = fz_resolve_link(doc->ctx, doc->document, str, NULL, NULL);
        pageNum = fz_page_number_from_location(doc->ctx, doc->document, loc);
    }
    fz_catch(doc->ctx)
    {
...
JNIEXPORT jint
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfLinks_fillPageLinkTargetPoint(
...
JNIEXPORT jint
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfDocument_getPageCount(JNIEnv *env,
                                                                      jclass clazz, jlong handle,
                                                                      ...
                                                                      jint size)
{
    ...
    fz_try(doc->ctx)
    {
        ...
        if (doc->accel && strlen(doc->accel) > 0) {
...
JNIEXPORT jlong
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_open(JNIEnv *env, jclass clazz,
                                                          jlong dochandle, jint pageno)
{
    ...
    if (!doc || !doc->ctx || !doc->document) {
        mupdf_throw_exception(env, "Invalid document handle");
        return (jlong)(long)NULL;
    }
    ...
    memset(page, 0, sizeof(renderpage_t));
...
JNIEXPORT void JNICALL
Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_free(JNIEnv
                                                          *env,
                                                      jclass clazz,
                                                      jlong
                                                          dochandle,
...
JNIEXPORT void JNICALL
Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_getBounds(JNIEnv
                                                               *env,
                                                           jclass clazz, jlong dochandle,
                                                           jlong handle, jfloatArray bounds)
{
    renderdocument_t *doc = (renderdocument_t *)(long)dochandle;
    renderpage_t *page = (renderpage_t *)(long)handle;
    
    if (!doc || !doc->ctx || !page || !page->page || !bounds)
        return;
    ...
    (*env)->ReleasePrimitiveArrayCritical(env, bounds, bbox,
                                          0);
}

JNIEXPORT void JNICALL
Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_renderPage(JNIEnv
                                                                *env,
                                                            jobject this, jlong dochandle,
                                                            jlong pagehandle, jintArray viewboxarray,
                                                            jfloatArray matrixarray, jintArray bufferarray,
                                                            jint r, jint g,
                                                            jint b)
{
    renderdocument_t *doc = (renderdocument_t *)(long)dochandle;
    ...
    ctm.e = matrix[4];
    ctm.f = matrix[5];
    (*env)->ReleasePrimitiveArrayCritical(env, matrixarray, matrix,
                                          0);
    ...
    (*env)->ReleasePrimitiveArrayCritical(env, viewboxarray, viewboxarr,
                                          0);

    buffer = (*env)->GetPrimitiveArrayCritical(env, bufferarray, 0);

    if (!doc || !doc->ctx || !page || !page->pageList) {
    ...
    (*env)->ReleasePrimitiveArrayCritical(env, bufferarray, buffer,
                                          0);
}

// Outline
JNIEXPORT jlong
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfOutline_open(JNIEnv *env,
                                                             jclass clazz, jlong dochandle)
{
    renderdocument_t *doc = (renderdocument_t *)(long)dochandle;
...
JNIEXPORT void JNICALL
Java_org_ebookdroid_droids_mupdf_codec_MuPdfOutline_free(JNIEnv
                                                             *env,
                                                         jclass clazz, jlong dochandle)
{
    renderdocument_t *doc = (renderdocument_t *)(long)dochandle;
                                                                 ...
                                                                 jlong outlinehandle)
{
    ...
    if (!doc || !doc->ctx || !outline || !outline->title)
    {
        return NULL;
    }
...
JNIEXPORT jstring
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfOutline_getLinkUri(JNIEnv *env,
...
JNIEXPORT jint
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfOutline_fillLinkTargetPoint(
...
JNIEXPORT jlong
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfOutline_getNext(JNIEnv *env,
                                                                jclass clazz, jlong outlinehandle)
{
    fz_outline *outline = (fz_outline *)(long)outlinehandle;
...
JNIEXPORT jlong
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfOutline_getChild(JNIEnv *env,
                                                                 jclass clazz, jlong outlinehandle)
{
    fz_outline *outline = (fz_outline *)(long)outlinehandle;
    //	DEBUG("MuPdfOutline_getChild(%p)",outline);

    if (outline && outline->down)
...
JNIEXPORT jobject
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_text116(JNIEnv *env,
...
JNIEXPORT jint
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfDocument_hasChangesInternal(JNIEnv *env,
                                                                            jclass clazz,
                                                                            jlong handle)
{
    renderdocument_t *doc_t = (renderdocument_t *)(long)handle;
...
JNIEXPORT void JNICALL
Java_org_ebookdroid_droids_mupdf_codec_MuPdfDocument_saveInternal(JNIEnv
                                                                      *env,
                                                                  jclass clazz, jlong handle,
                                                                  jstring fname)
{
    ...
    if (!doc_t || !doc_t->ctx || !doc_t->document || !fname) {
        return;
    }
...
JNIEXPORT void JNICALL
Java_org_ebookdroid_droids_mupdf_codec_MuPdfDocument_deletePage(JNIEnv
                                                                    *env,
                                                                jclass clazz, jlong handle,
                                                                jint page)
{
...
Java_org_ebookdroid_droids_mupdf_codec_MuPdfDocument_setMetaData(JNIEnv *env, jclass clazz, jlong handle, jstring jkey, jstring jvalue)
{
    ...
    if (!doc_t || !doc_t->ctx || !doc_t->document) return;
...
Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_addInkAnnotationInternal(JNIEnv
                                                                              *env,
                                                                          jobject thiz, jlong handle,
                                                                          jlong pagehandle, jfloatArray jcolors,
                                                                          jobjectArray arcs, jint width,
                                                                          jfloat alpha)
{
    renderdocument_t *doc_t = (renderdocument_t *)(long)handle;
    renderpage_t *page = (renderpage_t *)(long)pagehandle;
    ...
    (*env)->ReleasePrimitiveArrayCritical(env, jcolors, co,
                                          0);
...
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_getAnnotationsInternal(JNIEnv *env,
                                                                            jobject thiz, jlong handle,
                                                                            jlong pagehandle,
                                                                            jobjectArray arcs)
{
...
JNIEXPORT void JNICALL
Java_org_ebookdroid_droids_mupdf_codec_MuPdfDocument_deleteAnnotationInternal(JNIEnv
                                                                                  *env,
                                                                              jobject thiz, jlong handle,
                                                                              jlong pagehandle,
                                                                              int annot_index)
...
JNIEXPORT void JNICALL
Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_addMarkupAnnotationInternal(JNIEnv
                                                                                 *env,
                                                                             jobject thiz, jlong handle,
                                                                             jlong pagehandle, jobjectArray points,
                                                                             enum pdf_annot_type type, jobjectArray jcolors)
{
    renderdocument_t *doc_t = (renderdocument_t *)(long)handle;
    renderpage_t *page = (renderpage_t *)(long)pagehandle;
...
void fz_print_stext_block_as_html_my(fz_context *ctx, fz_output *out, fz_stext_block *block)
{
    if (!ctx || !out || !block) {
        return;
    }
    ...
    if (!block->u.t.first_line || !block->u.t.first_line->first_char) {
        return;
    }
    ...
    if (!block->u.t.last_line || !block->u.t.last_line->last_char) {
...
void fz_print_stext_page_as_html_my(fz_context *ctx, fz_output *out, fz_stext_page *page, int id)
{
    if (!ctx || !out || !page) {
        return;
    }
...
JNICALL  Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_getPageAsHtml(JNIEnv *env, jobject thiz,
                                                                   jlong handle, jlong pagehandle,
                                                                   jint jopts)
{
