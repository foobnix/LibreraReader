...
typedef struct
{
	...
	int page_count;
	const char **page;
	/* Librera: parsed ".ldir" index; doc->page entries point into it. */
	fz_xml_doc *ldir_xml;
} cbz_document;
...
/*
 * Librera: a ".ldir" file is not an archive but a small XML index describing a
 * "folder as book":
 *
 *	<container count="N">
 *	<item path="/path/to/image.jpg" />
 *	...
 *	</container>
 *
 * The paths are filesystem paths, so cbz_load_page falls back to fz_read_file
 * for entries that the containing directory archive does not hold.
 */
static int
cbz_stream_is_ldir(fz_context *ctx, fz_stream *stm)
{
	unsigned char buf[1024];
	size_t n, i;
	int64_t pos;
	int seen_container = 0;
	...
	fz_seek(ctx, stm, pos, SEEK_SET);

	for (i = 0; i + 10 <= n; i++)
	{
		if (!seen_container && !memcmp(buf + i, "<container", 10))
			seen_container = 1;
		else if (seen_container && !memcmp(buf + i, "<item", 5))
			return 1;
	}
...
static void
cbz_create_page_list_ldir(fz_context *ctx, cbz_document *doc, fz_stream *stm)
{
	fz_buffer *buf = NULL;
	fz_xml *container, *item;
	int count;
	...
	fz_try(ctx)
	{
		...
		doc->ldir_xml = fz_parse_xml(ctx, buf, 0);
	}
	fz_always(ctx)
	...
	container = fz_xml_find(fz_xml_root(doc->ldir_xml), "container");
	if (!container)
		fz_throw(ctx, FZ_ERROR_FORMAT, "cannot find container in ldir");

	count = fz_atoi(fz_xml_att(container, "count"));
	if (count < 0)
		count = 0;
	...
	item = fz_xml_find_down(container, "item");
	while (item && doc->page_count < count)
	{
		const char *path = fz_xml_att(item, "path");
		if (path)
...
cbz_open_document(fz_context *ctx, const fz_document_handler *handler, fz_stream *file, fz_stream *accel, fz_archive *dir, void *state)
{
	...
	fz_try(ctx)
	{
		if (file && cbz_stream_is_ldir(ctx, file))
		{
			/* Librera "folder as book": the directory holds the images. */
			doc->arch = fz_keep_archive(ctx, dir);
			cbz_create_page_list_ldir(ctx, doc, file);
		}
		else
		{
			if (file)
...
cbz_recognize_doc_content(fz_context *ctx, const fz_document_handler *handler, fz_stream *stream, fz_archive *dir, void **state, fz_document_recognize_state_free_fn **freestate)
{
	...
	fz_try(ctx)
	{
		...
		else if (cbz_stream_is_ldir(ctx, stream))
		{
			ret = 100;
			break;
		}
...
fz_document_handler cbz_document_handler =
{
	...
	1 /* wants_dir: Librera ".ldir" reads its images from the containing dir */
};
