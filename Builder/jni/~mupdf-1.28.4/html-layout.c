...
/* Librera: text-decoration rules are drawn thicker than the mupdf default. */
#define LIBRERA_DECORATION_LINEWIDTH 3
			...
			if (node->type == FLOW_WORD || node->type == FLOW_SPACE || node->type == FLOW_SHYPHEN)
			{
				...
				if (line)
				{
					if (line_ss == NULL)
					{
						...
						line_ss->linewidth = LIBRERA_DECORATION_LINEWIDTH;
					}
				}
