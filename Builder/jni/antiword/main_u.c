...
#define printf(...) __android_log_print(ANDROID_LOG_DEBUG, "Antiword", __VA_ARGS__);
...
Java_com_foobnix_libmobi_LibMobi_convertDocToHtml(JNIEnv *env, jclass clazz, jstring jinput, jstring joutpub) {
   ...
   char  *outpath = (char*) (*env)->GetStringUTFChars(env, joutpub, NULL);
    ...
    printf("Antiword in %s", fullpath);
    printf("Antiword out %s",outpath);
