...
Java_org_ebookdroid_droids_mupdf_codec_MuPdfLinks_getNextPageLink(JNIEnv *env,
    ...
    fz_link * link = (fz_link * )(
    long) linkhandle;
    return (jlong)(
    long) (link ? link->next : NULL);
}
...
y1;
(*env)->
ReleasePrimitiveArrayCritical(env, bounds, bbox,
0);
}
...
jobject this, jlong
dochandle,
jlong pagehandle, jintArray
viewboxarray,
jfloatArray matrixarray, jintArray
bufferarray,
jint r, jint
g,
jint b
...
f = matrix[5];
(*env)->
ReleasePrimitiveArrayCritical(env, matrixarray, matrix,
0);
...
y1 = viewboxarr[3];
(*env)->
ReleasePrimitiveArrayCritical(env, viewboxarray, viewboxarr,
0);
...
if(!ctx || ctx == NULL){
(*env)->
ReleasePrimitiveArrayCritical(env, bufferarray, buffer,
0);
return;
}
...
opts.
do_incremental = pdf_can_be_saved_incrementally(ctx, idoc);
...
jobject thiz, jlong
handle,
jlong pagehandle, jfloatArray
jcolors,
jobjectArray arcs, jint
width,
jfloat alpha
) {
renderdocument_t *doc_t = (renderdocument_t *) (long) handle;
...
(*env)->
ReleasePrimitiveArrayCritical(env, jcolors, co,
0);
...
pts[k].
x = jpt ? (*env)->GetFloatField(env, jpt, x_fid) : 0.0f;
pts[k].
y = jpt ? (*env)->GetFloatField(env, jpt, y_fid) : 0.0f;
(*env)->
DeleteLocalRef(env, jpt
);
...
jobject thiz, jlong
handle,
jlong pagehandle, jobjectArray
points,
enum pdf_annot_type type, jobjectArray
jcolors) {
renderdocument_t *doc_t = (renderdocument_t *) (long) handle;
renderpage_t *page = (renderpage_t *) (long) pagehandle;
...
JNICALL
Java_org_ebookdroid_droids_mupdf_codec_MuPdfPage_getPageAsHtml(JNIEnv *env, jobject thiz,
