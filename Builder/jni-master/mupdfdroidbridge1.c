...
Java_org_ebookdroid_droids_mupdf_codec_MuPdfLinks_getPageLinkTargetPage(
	...
	fz_location linkLoc =  fz_resolve_link(doc->ctx, link->doc, link->uri, NULL, NULL);
        int linkNumber = fz_page_number_from_location(doc->ctx, link->doc, linkLoc);
        return linkNumber;
}
...
Java_org_ebookdroid_droids_mupdf_codec_MuPdfLinks_getLinkPage(
	...
	fz_location linkLoc =  fz_resolve_link(doc->ctx, doc->document, str, NULL, NULL);
    int linkNumber = fz_page_number_from_location(doc->ctx, doc->document, linkLoc);
    return linkNumber;
}
